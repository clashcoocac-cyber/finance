...
def _recalc_report(report_id):
    ...
    if qs.exists():
        ...
        for tr in qs:
            ...
            # accumulate as Decimal (float + Decimal raises TypeError), store as
            # float for JSON
            uzs[key] = uzs.get(key, Decimal('0')) + (tr.amount_uzs or 0)
            usd[key] = usd.get(key, Decimal('0')) + (tr.amount_usd or 0)
            rub[key] = rub.get(key, Decimal('0')) + (tr.amount_rub or 0)
            eur[key] = eur.get(key, Decimal('0')) + (tr.amount_eur or 0)

        report.uzs_detail = {k: float(v) for k, v in uzs.items()}
        report.usd_detail = {k: float(v) for k, v in usd.items()}
        report.rub_detail = {k: float(v) for k, v in rub.items()}
        report.eur_detail = {k: float(v) for k, v in eur.items()}
    else:
